...
function artistIdFor(artist: string): number {
  return (
    [...artist].reduce(
      (sum, character) => (sum * 31 + (character.codePointAt(0) ?? 0)) % 2_000_000_000,
      0,
    ) + 1
  );
}
...
describe("rediscovery analysis", () => {
  it("handles exact 90, 180, and 365-day thresholds while excluding never-important artists", () => {
    withTemporarySqliteDatabase(({ connection }) => {
      ...
      for (const threshold of [90, 180, 365]) {
        const result = generateRediscoveryAnalysis({
          connection,
          parameters: { ...parameters, absenceThresholdDays: threshold },
          presentationTimezone: "America/Chicago",
        });
        assert.equal(
          result.result.rediscoveries.some((item) => item.entityDisplayName === "Minor"),
          false,
        );
        const expected =
          threshold === 90 ? "Ninety" : threshold === 180 ? "OneEighty" : "ThreeSixtyFive";
        assert.equal(
          result.result.rediscoveries.some((item) => item.entityDisplayName === expected),
          true,
        );
      }
    });
    ...
    withTemporarySqliteDatabase(({ connection }) => {
      ...
      const plays = [
        ...
        ...history(
          ...
          70,
          [
            0, 1, 2, 3, 4, 730, 731, 732, 733, 734, 735, 736, 737, 738, 739, 740, 741, 820, 821,
      ...
      assert.equal(
        returns.find((item) => item.entityDisplayName === "OneOff")?.classification,
        "one_off_return",
      );
      assert.equal(
        returns.find((item) => item.entityDisplayName === "Sustained")?.classification,
        "sustained_rediscovery",
      );
      const repeated = returns.filter((item) => item.entityDisplayName === "Repeated");
      assert.equal(repeated.length, 2);
      assert.equal(
        repeated.every((item) => item.persistence === "persistent"),
        true,
      );
  ...
  it("supports track scope independently of artist scope", () => {
    withTemporarySqliteDatabase(({ connection }) => {
      ...
      const result = generateRediscoveryAnalysis({
        connection,
        parameters: { ...parameters, scope: "track" },
        presentationTimezone: "America/Chicago",
      });
