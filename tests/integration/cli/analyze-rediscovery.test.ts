...
import { type TemporaryTestWorkspace, withTemporaryTestWorkspace } from "../../helpers/temporary-workspace.ts";

function runCli(workspace: TemporaryTestWorkspace, args: readonly string[]) {
  return spawnSync(process.execPath, [path.join(repositoryRoot, "src/cli/analyze-rediscovery.ts"), ...args], { cwd: repositoryRoot, encoding: "utf8", env: { ...process.env, MUSICOLOGY_DATABASE_PATH: workspace.configuration.paths.databasePath, MUSICOLOGY_TIMEZONE: "America/Chicago" } });
}
...
describe("analyze:rediscovery CLI", () => {
  it("emits the versioned result envelope and a concise human result", () => {
    withTemporaryTestWorkspace((workspace) => {
      ...
      const result = JSON.parse(json.stdout) as { readonly command: string; readonly data: { readonly analysis: string } };
  ...
  it("rejects invalid arguments with a safe usage result", () => {
    withTemporaryTestWorkspace((workspace) => {
      ...
      assert.equal((JSON.parse(result.stderr) as { readonly errors: readonly { readonly code: string }[] }).errors[0]?.code, "invalid_arguments");
    });
  });
});
